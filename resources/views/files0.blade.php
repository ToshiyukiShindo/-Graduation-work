...
@section('content')
    ...
    <div class="card-body">
        ...
        <!-- 投稿フォーム -->
        @if( Auth::check() )
        <form action="{{ url('/files0/upload') }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
            <!-- 投稿のタイトル -->
        ...
        <div class="card m-1" style="width: 18rem;">
          <img src="{{'uploads/'.$file->img_url}}" class="card-img-top font-weight-bold" style="height:200px;">
            <div class="card-body">
                ...
                <hr>
                <div class="d-flex">
                    <form action="{{ url('filedetail0/'.$file->id) }}" method="GET" name="id"> 
                    {{ csrf_field() }}
                        <button type="submit" class="btn btn-sm btn-info">詳細</button>
                    </form>
                    <form action="{{ url('files0/'.$file->id) }}" method="POST">
                    {{ csrf_field() }}
                    {{ method_field('DELETE') }}
                        <button type="submit" class="btn btn-sm btn-danger ml-1">削除</button>
                    </form>
                    <form action="{{ url('files0/'.$file->id) }}" method="GET" name="id"> 
                    {{ csrf_field() }}
        ...
        <div class="card m-1" style="width: 18rem;">
          <img src="{{'uploads/'.$storefile->img_url}}" class="card-img-top font-weight-bold" style="height:200px;">
            <div class="card-body">
                ...
                <hr>
                <div class="d-flex">
                    <form action="{{ url('filedetail0/'.$storefile->id) }}" method="GET" name="id"> 
                    {{ csrf_field() }}
                        <button type="submit" class="btn btn-sm btn-primary">詳細</button>
                    </form>
                    <form action="{{ url('files0/'.$storefile->id) }}" method="POST">
                    {{ csrf_field() }}
                    {{ method_field('DELETE') }}
                        <button type="submit" class="btn btn-sm btn-danger ml-1">削除</button>
                    </form>
